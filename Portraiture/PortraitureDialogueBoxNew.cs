...
//Begin changes
        ...
        public void drawPortrait(SpriteBatch b)
        {
            ...
            if (!isVanillaType)
            {
                ...
                if (setTexture(picked + "_" + index, texture))
                {
                ...
                if (foundAnimation == false && setTexture(picked + "_" + "all", texture))
                {
                    
                    animTexture = ImageHelper.pTextures[picked + "_" + "all"];
                    foundAnimation = true;
                }

                if (foundAnimation == false && setTexture(characterName + "_" + index, texture))
                {
                    
                    animTexture = ImageHelper.pTextures[characterName + "_" + index];
                    foundAnimation = true;
                }

                if (foundAnimation == false && setTexture(characterName + "_" + "all", texture))
                {
            ...
            if (hasAnimation && !animationFinished)
            {
                useIndex = frame;
            }
            ...
            if (!useTexture.Bounds.Contains(rectangle))
            {
            ...
            if(totalTick >= nextTick && ImageHelper.displayAlpha > 0)
            {
                ImageHelper.displayAlpha -= 0.05f;
            }
            ...
            string activeFolderText = ImageHelper.getFolderName();

            activeFolderText = activeFolderText.Replace('_', ' ');

            if (activeFolderText == "Favorites")
            {
            ...
            float displayAlpha = (ImageHelper.displayAlpha > 1) ? 1 : ImageHelper.displayAlpha;
