...
namespace CustomFurniture.Overrides
{
    ...
    public class FurnitureFix
    {
        internal static bool Prefix(Furniture __instance, SpriteBatch spriteBatch, Vector2 location, float layerDepth, float alpha = 1f)
        {
            ...
            {
                spriteBatch.Draw(ho.texture, location, new Rectangle?(ho.sourceRect), Color.White * alpha, 0.0f, Vector2.Zero, (float)Game1.pixelZoom, ho.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
                return false;
            }
