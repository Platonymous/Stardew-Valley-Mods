...
namespace CustomFurniture
{
    ...
    internal class PyTkMigrator
    {
        ...
        ****/
        ...
        public static void MigrateItems(string type, Func<IDictionary<string, string>, Item, Item> getReplacement)
        {
            // player items
        ...
        /// <param name="type">The custom type identifier.</param>
        /// <param name="getReplacement">Get the replacement for the given PyTK fields.</param>
        public static void MigrateFurniture(string type, Func<IDictionary<string, string>, Item, Item> getReplacement)
        {
            // items in locations
            foreach (GameLocation location in Game1.locations
                .Concat(
            ...
            {
                foreach (Furniture obj in location.furniture.ToList())
                {
                    if (PyTkMigrator.TryMigrate(obj, type, getReplacement, out Item replaceWith) && replaceWith is Furniture replaceWithObj)
                        location.furniture[location.furniture.IndexOf(obj)] = replaceWithObj;
                }
        ...
        *********/
        ...
        /// <param name="type">The custom type identifier.</param>
        /// <param name="getReplacement">Get the replacement for the given PyTK fields.</param>
        private static void TryMigrate(IList<Item> items, string type, Func<IDictionary<string, string>, Item, Item> getReplacement)
        {
        ...
        private static bool TryMigrate(Item item, string type, Func<IDictionary<string, string>, Item, Item> getReplacement, out Item replaceWith)
        {
            ...
            {
                replaceWith = getReplacement(customData, item);
                return true;
            }
        ...
        private static bool TryParseSerializedString(string serialized, out string type, out IDictionary<string, string> customData)
        {
            ...
            foreach (string field in fields.Skip(3))
            {
                ...
                customData[parts[0]] = parts.Length > 1 ? parts[1] : null;
            }
