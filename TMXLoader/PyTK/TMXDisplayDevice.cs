...
namespace TMXLoader
{
    public class PyDisplayDevice : IDisplayDevice
    {
        ...
        {
           if(invalidated)
                TMXLoaderMod.helper.GameContent.InvalidateCache(tileSheet.ImageSource);
            ...
            try
            {
                ...
                else if (string.IsNullOrWhiteSpace(Path.GetDirectoryName(tileSheet.ImageSource)))
                {
                    ...
                    {
                        TMXLoaderMod.monitor.Log("Could not load Tilesheet:" + Path.GetFileName(tileSheet.ImageSource), StardewModdingAPI.LogLevel.Trace);
                    }
                }
            }
            catch
