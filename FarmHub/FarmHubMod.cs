...
namespace FarmHub
{
    public class FarmHubMod : Mod
    {
        ...
        internal static string passwort => config.Passwort;
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            new ConsoleCommand("join", "Join a server by name. Name and passwort have to use _ instead of spaces. Usage: join [name] [passwort if required]", (s, p) => populateList((l) => findServer(l,p) )).register();
        }
        ...
        private void findServer(List<FarmHubServer> list, string[] p)
        {
            ...
            if (p.Length < 1)
                ...
                Monitor.Log("Too many arguments, make sure to use _ instead of spaces in the name and passwort", LogLevel.Error);
            ...
            if (pass.toMD5Hash() != server.Passwort)
            {
                Monitor.Log("Wrong passwort.", LogLevel.Error);
                return;
            }
        ...
        private void writeListToConsole(List<FarmHubServer> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                Monitor.Log($"{i} : {f.Name} ({f.CurrentPlayers}/{f.MaxPlayers}) [{ (f.Passwort == open ? "open" : "pw") }]", LogLevel.Info);
